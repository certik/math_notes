...
**Two routes to uniqueness.** The signed-permutation argument of Step 3 is needed only because the assumed symmetry is the *geometric* one — isometry invariance (I), which the metric cuts down to $O(g)$, strictly weaker than full $GL$. Trading (I) for a stronger *algebraic* hypothesis removes it. Pulling cyclicity back through the index-raising {eq}`eq-trmet-raise`, with $\tilde D(S):=D(GS)$ the cyclicity $\tilde D(S_1S_2)=\tilde D(S_2S_1)$ becomes
```{math}
:label: eq-trmet-metric-cyclicity

D\big(M_1\,G^{-1}M_2\big)=D\big(M_2\,G^{-1}M_1\big)\qquad(\forall\,M_1,M_2),
```
"metric-cyclicity" — the trace of a metric-product is symmetric in its two factors. Assuming **(L)** together with {eq}`eq-trmet-metric-cyclicity` in place of (I) makes $\tilde D$ a linear cyclic functional on ordinary matrices, so [Trace From Linearity and Cyclicity](trace_linear_cyclic.md) delivers $\tilde D=\lambda\operatorname{tr}$ at full $GL(V)$ at once, giving $D(M)=\lambda\,g^{ij}M_{ij}$ with no signed-permutation step. It is a genuine trade-off: the weaker geometric (I) needs Step 3, the stronger algebraic {eq}`eq-trmet-metric-cyclicity` inherits uniqueness directly. Either way uniqueness rests on an assumption — only the constructive definition {eq}`eq-trmet-quick` needs none.
