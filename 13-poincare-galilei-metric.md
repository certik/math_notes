...
$\{e^1, e^2\}$ in coordinates. For the embedding of $S^2$ of radius $R$
in $\mathbb{R}^3$, the Killing vector fields generated by $\mathfrak{m}$
work out (in spherical coordinates $(\theta, \phi)$ centred on the north
pole) to
...
These are *not* the coordinate vectors $\partial_\theta, \partial_\phi$,
but they span the same tangent plane at every interior point. The dual
orthonormal coframe is

$$
e^1 = R\, d\theta, \qquad e^2 = R\, \sin\theta\, d\phi,
$$

so that $g = \delta_{ab}\, e^a \otimes e^b = (e^1)^2 + (e^2)^2$, which
evaluates in coordinates to

$$
ds^2 = R^2 \,(d\theta^2 + \sin^2\theta\, d\phi^2),
$$

with $R^2$ proportional to $\lambda$. This is the **round metric** of
$S^2$ — and it is the *same* metric as $\delta_{ab}$ in the frame: the
two formulas express the same bilinear form in different bases. The
$\sin^2\theta$ is not a feature of the metric itself; it is the squared
length of the coordinate vector $\partial_\phi$, a Killing field whose
magnitude varies from $0$ at the poles to $R$ at the equator.
