...
### Why the centrifugal potential does not satisfy Poisson's equation

Specialize to constant $\omega$. Since

$$
\Gamma^x_{tt}=-\omega^2x,
\qquad
\Gamma^y_{tt}=-\omega^2y,
$$

one might try to regard the entire $\Gamma^i_{tt}$ as the gradient of an effective potential,
...
This does not violate the Newton–Cartan field equation. The rotating connection also contains the Coriolis coefficients
...
whose quadratic contribution cancels the divergence of $\Gamma^i_{tt}$:

$$
R_{tt}
=
\partial_i\Gamma^i_{tt}
-
\Gamma^j_{ti}\Gamma^i_{tj}
=
(-2\omega^2)-(-2\omega^2)
=
0.
...
The physical gravitational potential in this source-free example is $\Phi=0$, which does satisfy Poisson's equation. What fails is treating the centrifugal and Coriolis parts of a rotating connection as though they came from one gravitational scalar. This illustrates why $R_{\mu\nu}=4\pi G\rho\,\tau_\mu\tau_\nu$ is the fundamental tensor equation, whereas the scalar Poisson equation is obtained only after the inertial and gravitational parts of the connection have been separated.
