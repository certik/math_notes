...
As a check and a convenient package for the result, the same connection is obtained from
...
by computing its Levi-Civita connection in $(q^0,x,y,z)$ and taking $C\to\infty$.
...
## 3. General relativity in the same local-time chart
...
## 4. Common direct-coordinate pipeline

| Stage | Newton–Cartan | General relativity |
|---|---|---|
| Supplied chart | $(q^0,x,y,z)$ with smooth local-time offset $\sigma(x)$ | The same chart |
| Physical source | $\rho(r)$ and total mass $M$ | Spherical body with mass $M$; vacuum exterior |
| Fixed geometric data | $\tau=\mathrm dq^0-\mathrm d\sigma$ and $h^{\mu\nu}$ | None |
| Coordinate gauge | Components of $\tau,h^{\mu\nu}$ | Metric ansatz containing $\vartheta=\mathrm dq^0-\mathrm d\sigma$ |
| Dynamical variable | Compatible connection $\Gamma^\lambda_{\mu\nu}$ | Metric functions $A(r),B(r)$ |
| Tensor field equation | $R_{\mu\nu}=4\pi G\rho\,\tau_\mu\tau_\nu$ plus compatibility and Trautman | $G_{\mu\nu}=0$ outside the source |
| Result | Full NC connection in the local-time chart | Exact Schwarzschild geometry in the local-time chart |
| Motion | Affine geodesic with $\tau_\mu\dot q^\mu=1$ | Proper-time geodesic |
