...
The finite-$C$ Ricci tensor is most transparent in the coframe $(\vartheta,\mathrm dx^i)$. Its non-zero coefficients are

$$
R_{\vartheta\vartheta}[g^{(C)}]
=
...
The terms depending on $\sigma_i$ and $\sigma_{ij}$ cancel from these coframe coefficients because

$$
\mathrm d\vartheta=0.
$$

They remain present in individual coordinate-basis connection components, but they do not create curvature. Taking the formal limit gives
...
The cancellation of timezone-profile derivatives can be seen directly before evaluating the Einstein tensor. Since
...
the Cartan exterior derivative of the temporal coframe element is
...
=
\frac{cA'}{2\sqrt{A}}\,
\mathrm dr\wedge\vartheta.
$$

It contains $A'(r)$ but no $\sigma_i$ or $\sigma_{ij}$. The remaining coframe derivatives depend only on the spherical spatial metric. Thus the connection one-forms and curvature two-forms reduce directly to equations for $A(r)$ and $B(r)$; the large component terms produced by $\sigma$ cancel in the tensor curvature.
...
Substituting the full metric above directly into the Einstein tensor in the $(q^0,x,y,z)$ chart gives
...
All additional component equations, including those containing derivatives of $\sigma$, are satisfied simultaneously when these two equations hold. Their solution is
