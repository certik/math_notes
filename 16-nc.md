...
Outside the Sun, $T_{\mu\nu}=0$. Unlike the Newton–Cartan Poisson equation, Einstein's equation has independent gravitational degrees of freedom, so the mass alone does not select one spacetime. We restrict to the spherically symmetric exterior. Birkhoff's theorem then removes the apparent extra freedom by implying that this vacuum exterior is static.
